...
class UploadContainerTasksCreator():
    ...
    def create_upload_tasks(self, build_tasks: Dict[str, DockerCreateImageTask]):
        return {release_type: self._create_upload_task(release_type, build_task)
                for release_type, build_task in build_tasks.items()}

    def _create_upload_task(self, release_type: str, build_task: DockerCreateImageTask):
        required_task_info = self._create_required_task_info(build_task)
        return self.task.create_task_with_common_params(
            UploadContainerTask,
